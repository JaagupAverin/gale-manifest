...
def task_generate_clangd_file(target: Target, final_build_dir: Path) -> None:
    """Writes a .clangd file for the given target in gale root directory (for LSPs to find)."""
    clangd_file_content = textwrap.dedent(f"""
    # This file is auto-generated by gale for the latest built target ({target.name}).
    CompileFlags:
      CompilationDatabase: {final_build_dir}
